...
export default function AccountCategories({
  switchCreateCategories,
}: {
    switchCreateCategories: boolean,
}) {
  const { mutate } = useSWRConfig();
  const router = useRouter();

  const categories = dataCategories().data;
  const categoriesArr = dataCategories();
  ...
  const { data: deletingResponse } = useSWR(
    activationOfDeletion &&
      `http://localhost:3001/categories?id=${categoriesIndex}`,
    deleteFetch
  );

  const { data: updateResponse } = useSWR(
    activationOfUpdating &&
      `http://localhost:3001/categories?id=${categoriesIndex}&name=${categoryInputValue}`,
    updateFetch
  );

  const { data: createResponse } = useSWR(
    activationOfCreating &&
      `http://localhost:3001/categories?name=${categoryInputValue}`,
    createFetch
  );
  ...
  useEffect(() => {
    ...
    mutate("http://localhost:3000/account/marking")
